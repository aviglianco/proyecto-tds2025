...
func main() {
	...
	// Generate x86-64 assembly from IR and write to .s file
	asm := ir.GenerateX86(ir_code)
	asmPath := filepath.Join(resultsDir, baseName+".s")
	if err := os.WriteFile(asmPath, []byte(asm), 0644); err != nil {
		fmt.Fprintf(os.Stderr, "error writing assembly output: %v\n", err)
		os.Exit(1)
	}
	fmt.Println("x86 assembly written to:", asmPath)
}
